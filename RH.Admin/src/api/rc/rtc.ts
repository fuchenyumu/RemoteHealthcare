...
export const requestRtcToken = (
  consultationId: number,
  data?: { forceRefresh?: boolean }
) =>
  http.request<ApiResponse<RtcTokenResponse>>(
    "post",
    `${modulePrefix}/${consultationId}/token`,
    {
      data
    }
  );
...
export const getRtcRoom = (roomId: string) =>
  http.request<ApiResponse<RtcRoomStatus>>(
    "get",
    `${modulePrefix}/rooms/${roomId}`
  );
