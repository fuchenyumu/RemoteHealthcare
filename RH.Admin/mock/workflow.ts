...
export default [
  {
    url: '/api/v1/definition/definitions', method: 'get', response: () => {
      return { data: [{ id: 1, name: 'ConsultationWorkflow' }] };
    }
  },
  {
    url: '/api/v1/instance/:id/start', method: 'post', response: ({ url, body }) => {
  ...
  {
    url: '/api/v1/instance/:id/auditing', method: 'post', response: ({ url, body }) => {
  ...
  {
    url: '/api/v1/instance/:id/instance-detail', method: 'get', response: ({ url }) => {
      const id = Number(url.split('/')[2]);
