    ...
    public static IServiceCollection AddSqlSugarService(this IServiceCollection services)
    {
        ...
        SqlSugarScope sqlSugar = new(
            ...
            db =>
            {
                ...
                {
                    //if (!(sql.Contains("purest_background_job_record") || sql.Contains("_wf_") || sql.Contains("purest_request")  )) 
                    //{
                    //    Console.WriteLine(sql);//输出sql
                    //    if (pars != null)
                    //    {
                    //        Console.WriteLine(JsonConvert.SerializeObject(pars.Select(it => it.ParameterName + ":" + it.Value)));//输出参数
                    //    }
                    //}
